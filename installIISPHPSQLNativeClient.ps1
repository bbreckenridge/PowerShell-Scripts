...
#Begin installation
Write-Host "`n******************************************************************************`nWelcome to the IIS, PHP, SQL Native Client Installer.`n******************************************************************************"

$defaultURL = "MyWebsite"
...
Write-Host "`nInstallation could take several minutes please wait..."
...
$php_version = "5.3"
$php_install = "C:\php"
$php_log = "C:\php\phplog"
$php_temp = "C:\php\phptemp"
$web_root = "C:\wwwroot"
$web_log = "C:\wwwroot\wwwlogs"
...
$currentBinding = get-webbinding -name 'MyWebsite' | select bindingInformation
...
if ($currentBinding -match [regex]::escape($url)) {
	...
	set-ItemProperty 'IIS:\Sites\MyWebsite\' -name logFile.directory -value $web_log
	Set-ItemProperty 'IIS:\Sites\MyWebsite\' -name applicationPool -value CPTRAX
	...
	Write-Host "`n****************************************************************************************`nInstallation Process Completed.  Press any key to continue ...`n****************************************************************************************"
